...
ytop = 0,                  // Top of the visible client area
...
function tt_Htm(tt, t_id, txt)
{
	...
	var t_y = '<div id="' + t_id + '" style="position:absolute;z-index:1010; -moz-opacity: .9; filter: alpha(opacity=90); border: 2px solid #999999; -moz-border-radius:5px; padding: 2px; background-color: #333333;';
	...
	if (t_shw)
	{
		...
		{
			var t_opa = tt_n6? '-moz-opacity:0.9;' : tt_ie? 'filter:Alpha(opacity=90);' : '';
...
function tt_EvY(t_e)
{
	...
	else if (t_y > ylim - 12 && (t_y - tt_objH < ytop + 6 || (tt_dwn && t_y - tt_objH < ytop + 24)) ) {
		// The tooltip floats off the bottom when below the mousepointer, and 
		// floats off the top when above the mousepointer.  Keep the tooltip 
	...
	{
		// The tooltip floats off the bottom.  Move it above the mousepointer.
		t_y -= (tt_objH + 5);
		tt_dwn = false;
	}
	else
	{
		// The tooltip fits below the mousepointer.  Move it there.
		t_y += tt_offY;
		tt_dwn = true;
	}
	return t_y;
...
function tt_Show(t_e, t_id, t_above, t_delay, t_fix, t_left, t_offx, t_offy, t_static, t_sticky)
{
	...
	if (tt_obj)
	{
		...
		ylim = tt_Int(window.innerHeight || tt_db.clientHeight) +
			...
			tt_objH - tt_offY;
		ytop = tt_Int(window.pageYOffset || (tt_db? tt_db.scrollTop : 0) || 0);
